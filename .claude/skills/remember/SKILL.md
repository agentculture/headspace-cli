...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope headspace-cli
...
scope:** the default personal scope is **private** (`--scope headspace-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `headspace-cli`). Private records are served only to a recall in the same
