...
- **A per-workspace cancellation token, so the marker channel is unforgeable from inside the box rather than merely hard to guess.** Closes [#20](https://github.com/agentculture/headspace-cli/issues/20), which this release opened and then closed after its own adversarial test wave *and* the PR's automated review independently found the same hole. `create` mints 128 bits from `secrets.token_hex` and carries them on the **anchor container** as `headspace.cancel_token`; `stop` writes `<token>:<job_id>` into both markers; `run` requires the whole string verbatim, never parsed on the separator. A label is the right home for structural reasons: `stop` and `run` are separate processes that share no memory but both already hold the anchor, and a label is engine metadata no process inside any container can read — let alone a job, which runs in a different container again. Before this, the only thing between a job and a fabricated `cancelled` was its not knowing its own `job_id`, and `--job-id` is the caller's to choose: a caller who picked predictable ids *and* ran untrusted code had handed that code the one string it was missing. A live test forged a cancellation exactly that way; the same test, with the attack unchanged, now asserts the refusal. Two consequences, both pinned: a workspace whose anchor predates the token classifies no cancellation at all rather than falling back to bare job ids — the fallback *is* the hole — and a job's self-planted intent marker no longer buys it the settle wait, so closing the forgery closed a small denial-of-service lever with it. The boundary is job-versus-record and nothing more is claimed: whoever can reach the Docker socket can read the label, and whoever can reach `~/.headspace` can edit the record directly.
