...
headspace-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. headspace-cli
...
#   - consumer-identifying prose: `guildmaster` → `headspace-cli` (NOT
...
If a re-sync would lose a headspace-cli adaptation, lift the change
...
#     → `which headspace-cli vendors from guildmaster`
