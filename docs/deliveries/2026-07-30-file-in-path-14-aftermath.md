...
| `t7` | delivered | `76e7449`, merged into `HEAD`. Seven live probes (+936 lines). Three of its findings became code changes outside every task's scope (`d8`, `d9`, `d10`) and one — issue #20 — was opened by this delivery and then closed by it, after the PR's automated review independently found the same thing. |
...
- `d9` *(proposed)* — the module docstring's **"Unforgeable"** was an overclaim; the property was conditional on the job id's entropy. Restated at the point of use and filed as #20 — then closed by `d10`, so the docstring now states the unconditional property the token actually delivers.
- `d10` *(proposed)* — **closed #20 inside this delivery** rather than deferring it, after the PR's automated review independently found the same residual `t7` had. Implemented the reviewer's shape, which is better than the one I filed: #20's own candidate (1) proposed a per-run nonce held in `run`'s memory, which cannot work — `stop` is a different process. A per-workspace token on the anchor's labels is readable by both processes and by no container.
...
| `t7` (`d9`) | "Unforgeable" was stronger than what held; restated, filed as #20, then closed by `d10` | `acceptable` |
| `t7` (`d10`) | closed #20 in-delivery with a per-workspace token on the anchor label; outside every task's scope | `acceptable` |
...
- tests: `pytest -m 'not integration' -n 8` — **1006 passed** (baseline before this run: 976)
...
- PRs / issues: PR #21; closes #16, #18, #20; #15 closed by adjudication 2026-07-30; opened #19 (gate-2 deviations)
- review: SonarCloud 5 issues — 4 fixed, 1 (`S5655`) accepted with a written pushback after proving the premise; qodo 1 finding — fixed in full
...
| a job **cannot** forge a cancellation, even told its own `job_id` | high | `test_a_countersignal_a_job_planted_for_itself_is_refused` — the same attack that forged one before the workspace token, unchanged, now refused live · `test_a_countersignal_a_job_wrote_for_itself_is_refused` (4 payload shapes) |
| the token reaches the anchor and no job container, volume, or record | high | `test_the_cancellation_token_is_on_the_anchor_and_reaches_no_job` · `test_two_workspaces_never_share_a_cancellation_token` |
| a workspace predating the token classifies no cancellation rather than falling back | high | `test_a_workspace_with_no_token_classifies_no_cancellation_at_all` |
